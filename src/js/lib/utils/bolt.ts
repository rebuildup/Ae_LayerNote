...
// import type { Scripts } from "../../jsx/es-types/index";
// type Scripts = any; // Temporary type definition
...
export const evalTS = <Key extends string>(
  functionName: Key,
  ...args: any[]
): Promise<any> => {
...
export const initBolt = (log = true) => {
  try {
    ...
    // Ignore errors in non-CEP environments
    if (log) console.log('CEP not available, running in browser mode');
  }
};
